...
class Softmax(Module):
    ...
    Each output depends on every logit in its own row, not just the matching
    input.
    """
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        Sets:
            self.a (np.ndarray): the probabilities, saved for backward.
        """
        # subtract the row max so exp does not overflow
    ...
    def backward(self, grad_output: np.ndarray) -> np.ndarray:
        ...
        Args:
            grad_output (np.ndarray): gradient of the loss with respect to this
                layer's output, shape (batch_size, C).

        Returns:
            np.ndarray: gradient of the loss with respect to the logits, shape
            (batch_size, C).
        """
        # per row: a * (grad - sum(grad * a))
        summed = np.sum(grad_output * self.a, axis=1, keepdims=True)
        return self.a * (grad_output - summed)
